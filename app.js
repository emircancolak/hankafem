...
//  ⚠️ HARDCODED / TEKİL MÜŞTERİ SÜRÜMÜ ⚠️
//  Bu dosya, masalarında ESKİ URL'leri (GitHub Pages vb.) basılı olan tek
//  bir müşterinin eski deposuna yüklenmek üzere hazırlanmıştır. URL'de
//  ?kafe=... parametresi ARANMAZ — kafe kimliği doğrudan aşağıdaki
//  HARDCODED_KAFE_ID sabitinden okunur (bkz. ilgili bölüm).
// ════════════════════════════════════════════════════════
//
 ...
 *
 * NOT (HİBRİT SLIDER): "images/cilve3.mp4" gibi yerel/relative bir yol
 * verildiğinde `new URL(url)` mutlak bir adres olmadığı için hata fırlatır;
 * bu hata aşağıdaki catch bloğunda yakalanıp URL olduğu gibi (değiştirilmeden)
 * döndürülür. Yani bu fonksiyon yerel videolara/asseterlere DOKUNMAZ.
 *
 * @param {string} url
 * @returns {string}
 */
function toCdnUrl(url) {
  ...
  try {
    ...
    return url; // geçersiz/relative/parse edilemeyen URL — olduğu gibi bırak
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // PWA: Service Worker'ı kaydet ve çevrimdışı/çevrimiçi göstergesini başlat.
 ...
 *
 * @param {string} [shopName] - Biliniyorsa (settings yüklendiyse) manifest'in
 *   "name"/"short_name" alanlarını da işletmenin gerçek adıyla değiştirir.
 */
...
function applySettingsToDOM() {
  ...
  document.body.classList.toggle("layout-no-photo", !!_settings.noPhotoMode);
...
/**
 * Storage'dan 404/403 gibi bir hata gelirse sayfanın tamamını kaplar.
